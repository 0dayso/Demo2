awem = function ($) {
    ...
    function odropdown(o, opt) {
        ...
        function moveVal(val, afval) {
            var input = findvalinput(val);// $valCont.find('input[value="' + val + '"]');

            if (afval) {
                findvalinput(afval).after(input);
                //$valCont.find('input[value="' + afval + '"]').after(input);
            } else {
        ...
        function renderValInputs() {
            ...
            $.each(vals, function (i, item) {
                res += '<input type="' + valInputType + '" value="' + escape(item) + '" name="' + o.nm + '" checked="checked"/>';
            });
